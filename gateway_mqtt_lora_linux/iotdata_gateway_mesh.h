...
static inline void mesh_stat_frame(mesh_state_t *const st, const uint8_t ctrl, const int len, const bool ok) {
    if (st == NULL)
        return;
    if (ctrl >= MESH_CTRL_COUNT) { /* a runt: peek_ctrl_type could not read a type at all */
        st->stat_ctrl_runt++;
        return;
    }
    mesh_ctrl_stat_t *const c = &st->ctrl[ctrl];
    if (ok)
        c->rx++;
    else
        c->err++;
...
static inline uint32_t mesh_stat_total(const mesh_state_t *const s, const bool errors) {
    uint32_t n = 0;
    for (unsigned i = 0; i < MESH_CTRL_COUNT; i++)
        n += errors ? s->ctrl[i].err : s->ctrl[i].rx;
    return n;
}
...
void mesh_transmit_beacon(mesh_state_t *st) {
    ...
    if (st->packet_handler(buf, IOTDATA_MESH_BEACON_SIZE)) {
        st->stat_beacons_tx++;
        st->stat_bytes_tx += (uint64_t)IOTDATA_MESH_BEACON_SIZE;
    } else {
        st->stat_errors_tx++;
...
void mesh_transmit_ack(mesh_state_t *st, uint16_t origin_station, uint16_t origin_sequence) {
    ...
    if (st->packet_handler(buf, IOTDATA_MESH_ACK_SIZE)) {
        st->stat_acks_tx++;
        st->stat_bytes_tx += (uint64_t)IOTDATA_MESH_ACK_SIZE;
    } else {
        st->stat_errors_tx++;
...
bool mesh_receive_forward(mesh_state_t *st, const uint8_t *buf, int len, const uint8_t **inner, int *inner_len) {
    ...
    if (iotdata_mesh_unpack_forward(buf, len, &fwd)) {
        ...
        if (st->dedup_handler && !st->dedup_handler(st->dedup_handler_ctx, fwd.origin_station, fwd.origin_sequence)) {
            st->stat_duplicates++;
            if (st->debug)
                PRINTF_INFO("mesh: rx FORWARD duplicate suppressed origin={station=%04" PRIX16 ", sequence=%" PRIu16 "}, inner-length=%d\n", fwd.origin_station, fwd.origin_sequence, fwd.inner_len);
            /* still ACK to prevent the forwarder from retrying */
            if (st->enabled)
                mesh_transmit_ack(st, fwd.origin_station, fwd.origin_sequence);
            return false;
        }
        /* ACK the forwarder */
        if (st->enabled)
            mesh_transmit_ack(st, fwd.origin_station, fwd.origin_sequence);
        st->stat_forwards_unwrapped++;
...
void mesh_receive_beacon(mesh_state_t *st, const uint8_t *buf, int len) {
...
void mesh_receive_ack(mesh_state_t *st, const uint8_t *buf, int len) {
...
void mesh_receive_route_error(mesh_state_t *st, const uint8_t *buf, int len) {
...
void mesh_receive_neighbour_report(mesh_state_t *st, const uint8_t *buf, int len) {
...
void mesh_receive_pong(mesh_state_t *st, const uint8_t *buf, int len) {
    uint8_t variant;
