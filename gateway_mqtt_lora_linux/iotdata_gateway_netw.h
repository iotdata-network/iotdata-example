...
static inline void net_seq_track(net_pathstat_t *p, uint16_t seq) {
    ...
    if (delta == 0)
        return; /* same sequence again (a retransmit) — not a gap, do not advance */
        ...
        if (e->via_count > 0) {
            ...
            for (int s = 0; s < limit && (size_t)pos < outlen; s++) {
                ...
                if (best < 0)
                    break;
                used[best] = true;
                const int w = snprintf(out + pos, outlen - (size_t)pos, "%s0x%04" PRIX16 "=%" PRIu32, s ? "," : "", e->via[best].relay, e->via[best].count);
                if (w > 0)
                    pos += w;
            }
...
void network_report(const network_t *n, uint16_t gateway_id) {
    ...
    for (int i = 0; i < NETWORK_MAX; i++) { /* mesh nodes: gateways + relays */
        ...
        if (e->valid && (e->kind == NET_KIND_RELAY || e->kind == NET_KIND_GATEWAY)) {
            printf("  0x%04" PRIX16 " %-4s rssi=%ddBm age=%lds tx=%" PRIu32 "(gap=%" PRIu32 ",%.1f/min) beacons=%" PRIu32 " fwd=%" PRIu32 "(%.1f/min) acc=%c cost=%u gen=%u gw=0x%04" PRIX16 "\n", e->station, net_kind_name(e->kind), e->rssi,
                   ...
                   (unsigned)e->generation, e->gateway);
        }
    }
